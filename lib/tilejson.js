...
var bbox = function(zoom, x, y) {
  var tms_style = this.data.scheme === 'tms';
  // mercator knows only 2 projections and we're using spherical Mercator for
  // the tile system so we assume EPSG:3857 or equivalent is used as SRS.
  var srs = '900913';
  var nesw = mercator.bbox(x, y, zoom, tms_style, srs);
  var bboxStr = '' + nesw[0] + ',' + nesw[1] + ',' + nesw[2] + ',' + nesw[3];
  return bboxStr;
};
...
TileJSON.prototype.prepareURL = function(url, z, x, y) {
    // save ourselves some math if we don't need to generate a bbox
    if (url.match(/{bbox}/)) {
      this.bbox = bbox;
    } else {
      this.bbox = function() {};
    }
    this.sourceURI = this.sourceURI || {};
    this.sourceURI.query = this.sourceURI.query || {};
    this.scale=this.sourceURI.query.scale|| 1;
    this.data.autoscale = "autoscale" in this.data ? this.data.autoscale : true;

    if (this.scale > 1 && this.data.autoscale) {
      // replace the last "." with "@<scale>x."
      url = url.replace(/\.(?!.*\.)/, "@" + this.scale + "x.");
    }
    return url
        ...
        .replace(/\{bbox\}/g, this.bbox(z, x , y));
};
