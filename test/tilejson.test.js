...
(function() {
    ...
    tape('should load tile 0/0/0 through a WMS', function(assert) {
        osm_wms.getTile(0, 0, 0, function(err, data, headers) {
            ...
            assert.equal('image/png', headers['Content-Type']);
            assert.equal('string', typeof headers['ETag']);
            // it's the md5 of http://129.206.228.72/cached/osm?LAYERS=osm_auto%3Aall&SRS=EPSG%3A900913&FORMAT=image%2Fpng&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&STYLES=&BBOX=20037508.3384,20037508.3384,60112525.0168,60112525.0168&WIDTH=256&HEIGHT=256
